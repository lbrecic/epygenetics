...
class KnightClock(RegressionClock):
    """
    A specific implementation of the `RegressionClock` for the Knight clock.
    ...
    def __init__(self) -> None:
        """
        Initializes the KnightClock object by loading the necessary CpG sites
        and regression coefficients from a CSV file and setting the intercept
        for the regression model.
        """
        store: AbstractDataStore = CSVDataStore('data/CpGs')
        cpgs: Optional[pd.DataFrame] = store.retrieve_methylation_data(ClockType.KNIGHT)
