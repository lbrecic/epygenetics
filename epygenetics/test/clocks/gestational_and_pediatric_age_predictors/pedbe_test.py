...
class PEDBEClockTestCase(unittest.TestCase):

    @patch('pandas.read_csv')
    def test_initialization(self, mock_read_csv):
        """Test the initialization of PEDBEClock."""
        ...
        mock_read_csv.assert_called_once_with('data/CpGs/PEDBE_CpGs.csv')

        # Assert that the attributes are set correctly
        self.assertEqual(clock.name, ClockType.PEDBE)
        self.assertEqual(clock.marker_name, 'ID')
        self.assertEqual(clock.coef_name, 'Coef')
        self.assertEqual(clock.reg_coef, -2.10)
    ...
    def test_calculate(self, mock_read_csv, mock_anti_trafo):
        ...
        expected_tt = np.dot(dna_m[common_cpgs], mock_cpgs.set_index('ID').loc[common_cpgs, 'Coef']) - 2.10
