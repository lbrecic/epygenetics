...
class DNAmAgeCorticalClockTestCase(unittest.TestCase):

    @patch('pandas.read_csv')
    def test_initialization(self, mock_read_csv):
        """Test the initialization of DNAmAgeCorticalClock."""
        ...
        clock = DNAmAgeCorticalClock()

        # Assert that the CSV file was read correctly
        mock_read_csv.assert_called_once_with('data/CpGs/DNAmClockCortical_CpGs.csv')

        # Assert that the attributes are set correctly
        self.assertEqual(clock.name, ClockType.DNAM_AGE_CORTICAL)
        self.assertEqual(clock.marker_name, 'CpGs')
        self.assertEqual(clock.coef_name, 'coef')
        self.assertEqual(clock.reg_coef, 0.577682570446177)
    ...
    def test_calculate(self, mock_read_csv, mock_anti_trafo):
        ...
        expected_tt = np.dot(dna_m[common_cpgs], mock_cpgs.set_index('CpGs').loc[common_cpgs, 'coef']) + clock.reg_coef
        ...
        expected_pheno = pd.DataFrame({
            'Age': [30, 40],
            ClockType.DNAM_AGE_CORTICAL: [1.5, 2.5]
        })
