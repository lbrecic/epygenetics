...
class HorvathSkinAndBloodClockTestCase(unittest.TestCase):

    @patch('pandas.read_csv')
    def test_initialization(self, mock_read_csv):
        """Test the initialization of HorvathSkinAndBloodClock."""
        ...
        self.assertEqual(clock.reg_coef, -0.447119319)
    ...
    def test_calculate(self, mock_read_csv, mock_anti_trafo):
        ...
        expected_tt = np.dot(dna_m[common_cpgs], mock_cpgs.set_index('ID').loc[common_cpgs, 'Coef']) + clock.reg_coef
        ...
        expected_pheno = pd.DataFrame({
            'Age': [30, 40],
            ClockType.HORVATH_SKIN_AND_BLOOD: [1.5, 2.5]
        })
