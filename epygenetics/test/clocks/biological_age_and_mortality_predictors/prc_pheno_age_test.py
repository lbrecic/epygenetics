...
from epygenetics.clocks.biological_age_and_mortality_predictors.prc_pheno_age import PRCPhenoAgeClock
from epygenetics.clocks.type import ClockType


class PRCPhenoAgeClockTestCase(unittest.TestCase):

    @patch('pandas.read_csv')
    def test_initialization(self, mock_read_csv):
        """Test the initialization of PRCPhenoAgeClock."""
        ...
        clock = PRCPhenoAgeClock()

        # Assert that the CSV file was read correctly
        mock_read_csv.assert_called_once_with('data/CpGs/prcPhenoAge_CpGs.csv')

        # Assert that the attributes are set correctly
        self.assertEqual(clock.name, ClockType.PRC_PHENO_AGE)
