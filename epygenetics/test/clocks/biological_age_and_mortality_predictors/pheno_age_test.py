...
class PhenoAgeClockTestCase(unittest.TestCase):

    @patch('pandas.read_csv')
    def test_initialization(self, mock_read_csv):
        """Test the initialization of PhenoAgeClock."""
        ...
        mock_read_csv.assert_called_once_with('data/CpGs/PhenoAge_CpGs.csv')

        # Assert that the attributes are set correctly
        self.assertEqual(clock.name, ClockType.PHENO_AGE)
        self.assertEqual(clock.marker_name, 'CpG')
        self.assertEqual(clock.coef_name, 'Weight')
        self.assertEqual(clock.reg_coef, 60.664)
